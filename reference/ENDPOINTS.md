...
沒有寫解析器，不是忘了，是**沒看過真實回應**。照文件猜寫解析器正是這次移植
前期讓九張表錯六張的原因。要補上，先各抓一次並把回應給我：

```
twsix fetch-stock 5439 --sheet 三大法人 --save-html D:\tmp\djhtm
```

三大法人走的是同一批券商鏡像站（九張已驗證的分頁也走那裡），成功機率最高。
Goodinfo 與 MoneyLink 是另外的站，要另外處理。
