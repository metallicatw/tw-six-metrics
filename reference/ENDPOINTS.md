...
| 個股新聞 | ✅ | MoneyLink 富聯網，10 則，見 `twsix.ingest.news` |
| 股價(週) | ✅ | MoneyDJ `.djbcd`，1347 週，見 `twsix.ingest.weekly_prices` |
| 大戶持股 / 董監持股 | ❌ | Goodinfo 對機房 IP 回 403，帶 cookie 也一樣 |

### 河流圖：線是週的，分區是年的

一度以為活頁簿的週線來自鉅亨網，於是 `pending.py` 把 `股價(週)` 指到
`ps_historyprice.aspx`——那頁今天回的是 Next.js 外殼，沒有表格，抓回來當然是
「可疑」。真正的來源在 `Module1.MoneyDJ_TW_PRICE_New`：

```
{券商}/Z/ZC/ZCW/CZKC1_{代號}_W_1440.djbcd
```

也就是本專案其他九張表用的同一批 MoneyDJ 鏡像。回應不是 HTML，是六個以空白
分隔的區塊（日期／開／高／低／收／量），每塊以逗號分隔、等長、由舊到新。
5439 回 1347 週，最遠到 2000 年。

**線**用這個週收盤價，和活頁簿同一個來源。**分區**仍然來自交易所的年度收盤
平均價——這不是將就：分區邊界是「每年一個本益比倍數」的百分位，改成週取樣
只會讓成交週數多的年份被算好幾次。分區邊界是水平的，因為它們是今年的倍數
乘上今年的預估 EPS。

畫幾年由圖決定，不由資料決定：活頁簿的下拉選單預設回推七年（J3 = 2019），
本專案照抄。全部 1347 週畫進 720 單位寬的框，等於把二十年壓成底線、把要看的
三年擠進右邊八分之一。
...
### 〔年財務比率〕：那 0.3% 的來源，以及為什麼不是四捨五入

`fetch-stock` 現在也抓 `/z/zc/zcr/zcr0.djhtm?b=Y&a={代號}`。它是〔FRQ〕**同一張
頁面**的年表——〔FRQ〕自己的 `<SELECT>` 就指向這個網址——所以版面是同一套模板，
契約沿用〔FRQ〕的，假設錯了會直接報錯。

要它的原因不是精度潔癖。**年度EPS 不等於四季 EPS 相加**，而且差距不是四捨五入：

* 年度EPS = 全年淨利 ÷ **全年**加權平均股數
* 四季相加 = Σ(當季淨利 ÷ **當季**加權平均股數)

股數在年度中間變動時，兩者就會分開，而且差多少完全看增資規模。5439 的 110 年
就是這種情況——它的季 EPS 反推出 Q2 約 86.6 百萬股、Q4 約 88.3 百萬股——四季
相加是 4.67，公布的年度數字是 4.61。**0.06 是四個兩位小數四捨五入所能產生的
四倍以上。**

110 年剛好是五年本益比視窗裡「去掉最高最低後」的存活者之一，所以這個誤差
直接傳到整條區間：24.95 而不是 25.0179。抓了這張頁面之後，5439 的目標價從
391.7 變成 **392.8**——活頁簿的數字。

### 那三張還沒做的頁面，加上週線

沒有寫解析器，不是忘了，是**沒看過真實回應**。`twsix fetch-page` 就是去拿樣本的
那一個指令：

```
twsix fetch-page 5439 --save D:\tmp\pages
```

它知道每個來源的網址、編碼與需要的 header，而且**會先判斷回應能不能用再存**：

| 來源 | 分頁 | 判斷依據 |
|---|---|---|
| `holders` | 大戶持股 | Goodinfo，找「持股分級」 |
| `directors` | 董監持股 | Goodinfo，找「董監」 |

`prices` 與 `news` 已經畢業，改由 `fetch-stock` 一起抓，不再列在這裡。剩下
兩個都是 Goodinfo，而且卡的不是解析器：本專案在雲端跑，先進首頁存了 session
cookie、帶上 referer，兩個網址還是 403。從家用網路跑很可能兩張都拿得到。

那個判斷是為 Goodinfo 準備的：它擋 IP 的時候會回**一頁完整的、200 OK 的、
沒有表格的正常頁面**。把那個存下來當樣本、照著寫解析器，會得到一個「能跑、
但永遠回空」的解析器——比直接失敗更糟，因為沒有任何東西會說出來。
