...
#: 圖與表看同一段時間。
#:
#: 一度是 260（五年，Goodinfo 匯出的全部），而下面那張表列的是 52 週——同一頁上
#: 圖和表講不同的時間，讀者要自己換算。而且 260 個點的折線在一張 SVG 裡是 32 KB，
#: 兩張就 64 KB，佔一張完整版個股頁的五分之一。
#:
...
#:
#: 一度是 120（十年），理由是「董監持股是慢變數，短窗看不出換手」——那句話沒錯，
#: 但公開資訊觀測站的個股查詢也只回得出三年，所以那個窗有九成是空的，畫出來是
#: 一條貼著左邊界的線加上一大片留白。
DIRECTOR_MONTHS = 36
