...
def evaluate(
    ...
    if band is None:
        ...
        # 本益比估價是「本益比 × 每股盈餘」。EPS 是負的時候那個乘積是一個**負的
        # 股價**，而後面每一步都照算不誤：目標價 −68,720、下檔價 −31,046、
        # 報酬 −2.4、風險 −2.2，最後 `abs(ret / risk)` 把兩個負號約掉，得到
        # 報酬風險比 2.21 —— 一個看起來很漂亮的買進訊號。
        #
        # 一檔一檔看不會出事：畫面上同一行就寫著「預估EPS −1957」。全市場一起算
        # 才會出事——實測 1,958 檔裡有 166 檔 EPS 是負的卻拿到了報酬風險比，其中
        # 12 檔大於 2（潤隆、建漢、漢磊、富旺……）。而〔趨勢∩六大∩報酬〕那一頁
        # 正是拿 > 2 去篩的，所以那 12 檔會直接變成推薦名單。
        #
        # `PeBand.from_history` 本來就不拿 EPS ≤ 0 的年度去算區間（見它的 `ok`），
        # 這裡是把同一條規則補在缺掉的另一端。
        gaps["pe"] = "預估EPS為負，本益比估價不適用"
    else:
