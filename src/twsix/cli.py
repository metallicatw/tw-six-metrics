...
def cmd_refresh(args: argparse.Namespace) -> int:
    ...
    for code, was in queue:
        ...
        # 抓不齊**不代表算不出來**。實測 6614：十四張抓到十三張，六大指標要的七張
        # 全在，只有〔股利〕差一列——它只有九年的配息紀錄，而契約要求至少十列。
        # 那是一家年輕的公司，不是版面改版，而〔股利〕餵的是〔殖利率估價〕那一頁，
        # 不是六大指標。
        #
        # 舊的判斷在這裡就把它丟掉，於是它每一批都被重抓一次、每一批都失敗——
        # 一輪 82 檔失敗裡的絕大多數是這個。改成讓**評等引擎**當裁判：算得出六大
        # 指標就寫進清單，算不出來才算失敗。少一頁估值，比少一檔股票好。
        if rc != EXIT_OK:
            print("  （有幾張沒抓到，先看看六大指標算不算得出來）")
...
def cmd_backfill_prices(args: argparse.Namespace) -> int:
    ...
    每日快照是排程上線之後才開始累積的，所以〔外資投信〕圖上那條股價線一開始
    只有幾天——那張圖的視窗是 20 個交易日，不回補就要等四個星期才長齊。

    只補**還沒有**的日期，而且是**合併不覆蓋**：已經抓齊的那一天不會被一份少了
    半個市場的回補蓋掉。

    非交易日兩個交易所都回空表，所以「拿到零列」的意思是那天沒開市，跳過就好，
    ...
    for _ in range(want * 2 + 20):
        ...
        for d2, group in sorted(by_date(rows).items()):
            ...
            store.write_gz(
                f"market/daily/{folder}/{d2}", group, PRICE_COLUMNS,
                sort_by=("code",),
            )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    bp = sub.add_parser(
        "backfill-prices",
        help="回補過去 N 個交易日的全市場收盤（只補還沒有的日期）",
    )
