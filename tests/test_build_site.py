...
def test_the_index_row_is_the_shape_the_script_reads():
    """[代號, 名稱, 產業, 綜合評分, 更新日期, 抓取時間戳, 已下市] — a contract.
    ...
    # 第七欄是「已下市」。清單上看不到它了，但搜尋仍然找得到——所以搜尋結果
    # 自己要說得出來，否則點進去才發現，會讀成這個網站的資料錯了。
    assert row[6] == 0
...
def test_the_stock_page_is_tabs_rather_than_one_long_scroll():
    ...
    # 就是一段永遠打不開的內容。
    # 只數頁首那一排（`role="tab" id="tab-…"`）：〔財務健診〕裡面的子分頁也是
    # role="tab"，但它們對應的是子面板，不是 class="panel"。
...
def test_the_report_is_copied_in_before_the_build_and_a_failure_is_not_fatal():
    ...
    # 因為別的理由重建才會換，最差落後一整天。
    #
    # 週幾跟著上游：2026-09-23 起上游是 `1-5`（UTC 週一～週五 ＝ 台北週二～週六）
    # ——台北週一早上那一班抓到的是週五收盤，和週六那一班一模一樣，所以拿掉了；
    # 週五那一班（台北週六早上，週末看得到週五的美股）留著。
    #
    # 緩衝夠不夠、有沒有撞在 GitHub 的尖峰上，由 tests/test_schedule.py 守。
    pages = (repo / ".github/workflows/pages.yml").read_text("utf-8")
    assert 'cron: "37 23 * * 1-5"' in pages
